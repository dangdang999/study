...
class Wxcard_DeamFoodComModel extends ComModel 
{
	...
	protected function getUrl($do, $query = NULL) 
	{
		...
		if (strexists($url, '/addons/deam_food/')) 
		{
			$url = str_replace('/addons/deam_food/', '/', $url);
		}
	...
	public function wxCardUpdateImg($url,$uniacid) 
	{
		$account = m('common')->getAccount($uniacid);
	...
	public function createCard($params,$uniacid) 
	{
		$card_type = $params['card_type'];
		$logo_url = $params['wxlogourl'];
		$brand_name = $params['brand_name'];
		$code_type = 'CODE_TYPE_ONLY_BARCODE';
		$title = $params['title'];
		$color = $params['color'];
		if (empty($color)) 
		{
		...
		if(!empty($params['custom_app_brand_user_name'])){
			$custom_app_brand_user_name = $params['custom_app_brand_user_name'];
			$custom_app_brand_pass = "deam_food/pages/index/index";
			$activate_app_brand_user_name = $params['custom_app_brand_user_name'];
			$activate_app_brand_pass = "deam_food/pages/mine/activate";
		...
		if(!empty($custom_app_brand_user_name)){
			// $jsonData .= ',"custom_app_brand_user_name":"' . $custom_app_brand_user_name . '"';
			
			// $jsonData .= ',"custom_app_brand_pass":"' . $custom_app_brand_pass . '"';
		}
		if (!(empty($setpromotion))) 
		{
		...
		else if ($card_type == 'MEMBER_CARD') 
		{
			...
			if (!(empty($custom_field1))) 
			{
				...
				$jsonData .= ',"url":"http://www.baidu.com"';
				if(!(empty($custom_app_brand_user_name))){
					$jsonData .= ',"app_brand_user_name":"' . $custom_app_brand_user_name . '"';
					$jsonData .= ',"app_brand_pass":"deam_food/pages/mine/balance"';
				}
				$jsonData .= '}';
			}
			...
			// }
			if(!empty($custom_app_brand_user_name)){
				// $jsonData .= ',"activate_app_brand_user_name":"' . $activate_app_brand_user_name . '"';
				// $jsonData .= ',"activate_app_brand_pass":"' . $activate_app_brand_pass . '"';
			}
			if (!(empty($custom_cell1))) 
			{
	...
	public function updateCard($params,$uniacid) 
	{
		...
		$jsonData .= ',"code_type": "CODE_TYPE_ONLY_BARCODE"';
		$jsonData .= ',"use_limit":' . $use_limit;
		$jsonData .= ',"get_limit":' . $get_limit;
		$jsonData .= ',"can_share":' . $can_share;
		$jsonData .= ',"can_give_friend":' . $can_give_friend;
	...
	public function wxCardGetQuantity($cardid,$uniacid) 
	{
		$result = $this->wxCardGetInfo($cardid,$uniacid);
	...
	public function wxCardGetInfo($cardid,$uniacid) 
	{
	...
	public function wxCardModifyStock($uniacid,$card_id, $num, $type) 
	{
		$jsonData = '{';
		$jsonData .= '"card_id":"' . $card_id . '"';
		if (empty($type)) 
	...
	public function wxCardDelete($card_id,$uniacid) 
	{
		$jsonData = '{"card_id":"' . $card_id . '"}';
		$account = m('common')->getAccount($uniacid);
		if (method_exists($account, 'fetch_token')) 
	...
	public function wxCardGetQrcodeUrl($cardid,$uniacid) 
	{
	...
	public function ActivateMembercardbyopenid($uid) 
	{
		global $_W;
	...
	public function wxMembercardActivate($params,$uniacid) 
	{
		$code = $params['code'];
		$card_id = $params['card_id'];
	...
	public function wxMembercardUpdateuser($params,$uniacid) 
	{
		$code = $params['code'];
		$card_id = $params['card_id'];
		$bonus = $params['bonus'];
		...
		$is_notify_custom_field1 = $params['is_notify_custom_field1'];
		$jsonData = '{';
		$jsonData .= '"code":"' . $code . '"';
		$jsonData .= ',"card_id":"' . $card_id . '"';
		if (!(empty($bonus))) 
		...
		$jsonData .= ',"notify_optional":{"is_notify_bonus": true,"is_notify_balance": true,"is_notify_custom_field1":true}';
		if($is_notify_custom_field1){
	...
	public function membercardmanager($carddata,$uniacid, $card_id = '') 
	{
		$params = array('card_type' => 'MEMBER_CARD', 'title' => $carddata['card_title'], 'brand_name' => $carddata['card_brand_name'], 'quantity' => $carddata['card_quantity'], 'wxlogourl' => $carddata['card_logowxurl'], 'prerogative' => $carddata['prerogative'], 'notice' => '点击使用按钮并向服务员出示二维码', 'can_give_friend' => 0, 'description' => $carddata['card_description'], 'BIZ_SERVICE_FREE_WIFI' => intval($carddata['freewifi']), 'BIZ_SERVICE_WITH_PET' => intval($carddata['withpet']), 'BIZ_SERVICE_FREE_PARK' => intval($carddata['freepark']), 'BIZ_SERVICE_DELIVER' => intval($carddata['deliver']), 'datetype' => 'DATE_TYPE_PERMANENT', 'custom_cell1' => $carddata['custom_cell1'], 'custom_cell1_name' => $carddata['custom_cell1_name'], 'custom_cell1_tips' => $carddata['custom_cell1_tips'], 'custom_cell1_url' => $carddata['custom_cell1_url'], 'custom_field1' => 1, 'custom_field_name_type1' => 'FIELD_NAME_TYPE_LEVEL', 'supply_bonus' => 'true', 'bonus_url' => "", 'supply_balance' => empty($carddata['card_supply_balance']) ? 'false' : 'true', 'balance_url' => "", 'auto_activate' => 'false', 'wx_activate' => 'true','custom_app_brand_user_name'=>$carddata['wxapp_ghid']);
		...
		if (empty($card_id)) 
		{
			return $this->createCard($params,$uniacid);
		}
		$params['card_id'] = $card_id;
		return $this->updateCard($params,$uniacid);
	}
	...
	//
	public function updateActivateuserform($jsonData,$uniacid){
		$account = m('common')->getAccount($uniacid);
		if (method_exists($account, 'fetch_token')) 
		{
			$token = $account->fetch_token();
	...
	public function getMembercardUserInfo($jsonData,$uniacid){
		$account = m('common')->getAccount($uniacid);
		if (method_exists($account, 'fetch_token')) 
		{
			$token = $account->fetch_token();
