...
if($operation == 'display'){
	...
	empty($class) && itoast("请先添加分类！",$this->createWebUrl('goods',array('op'=>'class','method'=>'list','store_id'=>$store_id, 'version_id'=>intval($_GPC['version_id']))),'error');
	$id = intval($_GPC['id']);
	if(!empty($id)){
		$goods = pdo_get("deamx_food_goods",array('uniacid'=>$uniacid,'id'=>$id,'store_id'=>$store_id));
		empty($goods) && itoast("商品不存在或已被删除",$this->createWebUrl('goods',array('store_id'=>$store_id, 'version_id'=>intval($_GPC['version_id']))),'error');
	...
	if(checksubmit('submit') || checksubmit('submitadd')){
		...
		if(checksubmit('submit')){
			itoast('商品更新成功！',$this->createWebUrl('goods',array('store_id'=>$store_id, 'version_id'=>intval($_GPC['version_id']))), 'success');
		}elseif(checksubmit('submitadd')){
			itoast('商品更新成功！',$this->createWebUrl('goods',array('store_id'=>$store_id, 'op' => 'post', 'version_id'=>intval($_GPC['version_id']))), 'success');
		}
	...
	itoast('商品删除成功！',$this->createWebUrl('goods',array('store_id'=>$store_id, 'version_id'=>intval($_GPC['version_id']))), 'success');
	...
	if($method == 'list'){
		if(checksubmit('submit')){
			if (!empty($_GPC['sortid'])) {
		        ...
		        itoast('排序更新成功！',$this->createWebUrl('goods',array('op'=>'class','method'=>'list','store_id'=>$store_id, 'version_id'=>intval($_GPC['version_id']))), 'success');
		    }
		}
		$pindex = max(1, intval($_GPC['page']));
		$psize = 20;
		...
		if(checksubmit()){
			...
			if(!empty($id)){
				...
				itoast("分类更新成功！",$this->createWebUrl('goods',array('op'=>'class','method'=>'list','store_id'=>$store_id, 'version_id'=>intval($_GPC['version_id']))),"success");
			}else{
				$result = pdo_insert("deamx_food_class",$postArr);
				itoast("添加分类成功！",$this->createWebUrl('goods',array('op'=>'class','method'=>'list','store_id'=>$store_id, 'version_id'=>intval($_GPC['version_id']))),"success");
			}
		}
	}elseif($method == 'delete'){
		$id = intval($_GPC['id']);
		$goods = pdo_getall("deamx_food_goods",array('uniacid'=>$uniacid,'store_id'=>$store_id,'class_id'=>$id),array('id'));
		!empty($goods) && itoast("请将分类下的商品移除后再来删除分类！",$this->createWebUrl('goods',array('op'=>'class','method'=>'list','store_id'=>$store_id, 'version_id'=>intval($_GPC['version_id']))),'error');
		pdo_delete("deamx_food_class",array('uniacid'=>$uniacid,'store_id'=>$store_id,'id'=>$id));
		itoast("分类删除成功！",$this->createWebUrl('goods',array('op'=>'class','method'=>'list','store_id'=>$store_id, 'version_id'=>intval($_GPC['version_id']))),"success");
	}
}elseif($operation == 'tpl'){
	if($method == 'spec'){
		$spec = array('id' => random(32), 'title' => $_GPC['title']);
		include $this->template('system/store/tpl/spec');
		exit();
	}elseif($method == 'specitem'){
		$spec = array('id' => $_GPC['specid']);
		$specitem = array('id' => random(32), 'title' => $_GPC['title'], 'show' => 1);
		include $this->template('system/store/tpl/spec_item');
		exit();
	}elseif($method == 'attr'){
        $goods_attr = array('id' => random(32), 'title' => $_GPC['title']);
        include $this->template('system/store/tpl/attr');
        exit();
    }elseif($method == 'attr_item'){
        $goods_attr = array('id' => $_GPC['specid']);
        $options = array('id' => random(32), 'title' => $_GPC['title'], 'status' => 1);
        include $this->template('system/store/tpl/attr_item');
        exit();
    }elseif($method == 'new_specs'){
        include $this->template('system/store/tpl/new_specs');
        exit();
    }elseif($method == 'add_new_specs_item'){
        include $this->template('system/store/tpl/new_specs_item');
        exit();
    }

}

include $this->template('system/store/goods');
?>