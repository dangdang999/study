...
(function () {
    ...
    DeamUI.mask = {
        ...
        show: function (transparent, clickCallback) {
            ...
            var mask = $('.dui-mask');
            if (mask.length <= 0) {
                mask = $("<div class='dui-mask " + (transparent ? 'dui-mask-transparent' : '') + "'></div>");
        ...
        hide: function () {
            ...
            $('.dui-mask').removeClass('visible').transitionEnd(function (e) {
                $('.dui-mask').remove();
            });
        }
    };
    DeamUI.toast = {
        show: function (text, center, duration) {
            if ($('.dui-toast').length > 0) {
                $('.dui-toast').remove();
            }
            var toast = $("<div class='dui-toast'><span class='dui-toast-text'>" + text + "</span></div>");
        ...
        hide: function () {
            $('.dui-toast').addClass('out').transitionEnd(function (e) {
                $('.dui-toast').remove();
            });
        }
    };
    DeamUI.alert = function (text, title, callback) {

        DeamUI.dialog({
            title: title,
            text: text,
            buttons: [{
                text: DeamUI.defaults.confirmText,
                onclick: callback
            }]
        });
    };

    DeamUI.confirm = function (text, title, confirmCallback, cancelCallback) {
        ...
        DeamUI.dialog({
            title: title,
            text: text,
            extraClass: 'dui-dialog-confirm',
            buttons: [{
                text: DeamUI.defaults.cancelText,
    ...
    DeamUI.prompt = function (text, title, placeholder, callback) {
    ...
    var DeamUIModal = function (params) {
        ...
        if (mask) {
            DeamUI.mask.show(maskTransparent, maskClick);
        }

        self.container = $("<div class='dui-modal " + extraClass + "'>" + content + "</div>");
        ...
        self.show = function () {
            ...
            if (self.container.hasClass('dialog-modal')) {
                ...
                var dialog = self.container.find('.dui-dialog');
                dialog.css({
    ...
    window.DeamUIModal = DeamUIModal;

    DeamUI.dialog = function (params) {
        params = params || {};

        var titleHTML = params.title ? '<div class="dui-dialog-title">' + params.title + '</div>' : '';
        var textHTML = params.text ? '<div class="dui-dialog-text">' + params.text + '</div>' : '';

        var buttonsHTML = '';
        if (params.buttons && params.buttons.length > 0) {
            buttonsHTML += '<div class="dui-dialog-btns">';
        ...
        var promptHTML = '';
        if (params.prompt) {
            promptHTML += '<div class="dui-dialog-prompt"><input type="text" class="dui-dialog-prompt-input" placeholder="' + (params.prompt.placeholder ? params.prompt.placeholder : '') + '" value="' + (params.prompt.value ? params.prompt.value : '') + '"/></div>';
        }

        var dialogHTML = '<div class="dui-dialog  ' + (params.extraClass ? params.extraClass : '') + '">';
        dialogHTML += titleHTML + textHTML + promptHTML + buttonsHTML;
        dialogHTML += '</div>';
        DeamUI.dialog.modal = new DeamUIModal({
            content: dialogHTML,
            extraClass: "dialog-modal"
        });

        DeamUI.dialog.modal.show();

        $('.dui-dialog-btns a', DeamUI.dialog.modal.container).each(function (index, el) {
            $(el).click(function (e) {

                DeamUI.dialog.modal.close();

                if (params.buttons[index].onclick) {
                    if (params.prompt) {
                        params.buttons[index].onclick(DeamUI.dialog.modal.container, DeamUI.dialog.modal.container.find(':input').val(), e);
                        return;
                    }
                    params.buttons[index].onclick(DeamUI.dialog.modal, e);
                }
            });
        });
        return DeamUI.dialog.modal;
    };

    DeamUI.loader = {
        show: function (text, iconClass, duration) {
            DeamUI.mask.show(true);

            var loader = "<div class='dui-loader " + (text != 'mini' ? 'dui-loader-tip' : '') + "'>";
            if (text == 'mini' || text == 'loading') {
                loader += "<span class='dui-preloader dui-preloader-white'></span>";
                if (text == 'loading') {
                    loader += "<span class='dui-loader-text'>" + DeamUI.defaults.loaderText + "</span>";
                }
            } else {

                if (iconClass) {
                    loader += "<span class='dui-loader-icon'><i class='" + iconClass + "'></i></span>";
                }
                loader += "<span class='dui-loader-text'>" + text + "</span>";
            }
            loader += "</div>";
    ...
    DeamUI.notify = {
        show: function (text, title, css, autoClose) {
            ...
            var notifyHTML = "<div class='dui-notify dui-notify-" + css + "'>";
            if (title) {
                notifyHTML += "<span class='dui-notify-title'>" + title + "</span>";
            }
            if (text) {
                notifyHTML += "<span class='dui-notify-text'>" + text + "</span>";
            }
            notifyHTML += "<span class='dui-notify-close'>X</span>";
            notifyHTML += "</div>";

            DeamUI.notify.modal = new DeamUIModal({
                content: notifyHTML,
                extraClass: 'notify-modal',
                mask: false
            });
            DeamUI.notify.modal.show();
            DeamUI.notify.modal.container.find('.dui-notify-close').click(function () {
                DeamUI.notify.modal.close();
            });
    ...
    DeamUI.list = {
        init: function () {
            $(".dui-list").each(function () {

                $(this).find('.dui-checkbox,.dui-radio').each(function () {
                    var box = $(this);
                    var list = box.closest('.dui-list');
                    list.unbind('click').click(function () {

                        if (box.hasClass('dui-radio')) {
                            if (!box[0].checked) {
                                box[0].checked = true;
                            }
                            return;
    ...
    DeamUI.message = {
        show: function (params) {
            ...
            var messageHTML = "<div class='dui-message dui-message-popup'>" + iconHTML + titleHTML + contentHTML + buttonsHTML + "</div>";

            DeamUI.message.modal = new DeamUIModal({
                content: messageHTML,
                ...
                maskClick: function () {
                    if(!btn.holdModal){
                        DeamUI.message.modal.close();
                    }
                }
            });

            $('.dui-message-popup a', DeamUI.message.modal.container).each(function (index, el) {
                $(el).click(function (e) {
                    ...
                    if (params.buttons) {
                        if (params.buttons[index].onclick) {
                            params.buttons[index].onclick(DeamUI.message.modal, e);
                        }
                    }
                });
            });

            DeamUI.message.modal.show();
        },
        ...
        hide: function () {
            ...
            $('.dui-message-popup').addClass('out').transitionEnd(function (e) {
                $('.dui-message-popup').remove();
            });
            DeamUI.mask.hide();
        }
    };
    ...
    var DeamUIUploader = function (element, params) {
        var self = this;
        var defaults = {
            max: 1,
            uploadUrl: '',
        ...
        self.container = self.uploader.prev('.dui-images');
        ...
        if (!self.params.uploadUrl) {
            console.warn('DeamUI: UploadUrl not defined.');
            return;
        }
        ...
        self.file.change(function () {
            ...
            if(self.params.max>0){
                ...
                if(filecount>self.params.max){
                    DeamUI.toast.show('最多上传 ' + self.params.max  + ' 张图片!');
                    return;
                }
            }
                    ...
                    if(res.files){
                        ...
                        $.each(res.files,function(i){
                            ...
                            if(file.status!='success'){
                                DeamUI.toast.show(file.message);
                            }
                            else{
    ...
    $.fn.uploader = function (params) {
        ...
        return this.each(function () {
            ...
            if (!uploader) {
                uploader = new DeamUIUploader(this, params || {});
    ...
    var DeamUINumbers = function (element, params) {
        ...
        self.checkNumber = function (num) {
            ...
            if(self.params.canzero==1)
            {
                ...
                if (self.params.min > 0 && num <= self.params.min) {
                    ...
                    if (self.params.minToast && num <= self.params.min) {
                        DeamUI.toast.show(self.params.minToast.replace("{min}", self.params.min));
                    }
                    num = self.params.min;
            ...
            if (self.params.max > 0 && num >= self.params.max) {
                if (self.params.maxToast && num >= self.params.max) {
                    DeamUI.toast.show(self.params.maxToast.replace("{max}", self.params.max));
                }
                num = self.params.max;
    ...
    $.fn.numbers = function (params) {
        ...
        return this.each(function () {
            ...
            if (!numbers) {
                numbers = new DeamUINumbers(this, params || {});
    ...
    var DeamUIActionSheet = function (params) {
        var self = this;
        var cancelButton = {
            text: DeamUI.defaults.cancelText,
            extraClass: 'cancel'
        ...
        self.show = function () {
            ...
            if (self.params.title) {
                titleHTML = '<div class="dui-actionsheet-title">' + self.params.title + "</div>";
            }
            var actionSheetHTML = "<div class='dui-actionsheet " + (self.params.round ? 'dui-actionsheet-o' : '') + "'>" + titleHTML + buttonsHTML + "</div>";
            self.modal = new DeamUIModal({
            ...
            $('.dui-actionsheet a', self.modal.container).each(function (index, el) {
                $(el).click(function (e) {
    ...
    DeamUI.actionsheet = {
        ...
        show: function (title, buttons, round, clickClose) {
            var actionsheet = new DeamUIActionSheet({
                title: title,
                buttons: buttons,
                round: round,
    ...
    DeamUI.according = {
        init: function () {
            $('.dui-according').each(function () {
                var self = $(this);
                var header = self.find('.dui-according-header');
                var group = self.closest('.dui-according-group');
                header.unbind("click").click(function () {
                    group.find('.dui-according').not(self).each(function () {
                        DeamUI.according.collapse($(this).closest('.dui-according'));
                    });
                    if (self.hasClass('expanded')) {
                        DeamUI.according.collapse(self);
                    } else {
                        DeamUI.according.expand(self);
                    }
        ...
        expand: function (according) {
            var content = according.find('.dui-according-content');
            //content.css('height', 'auto');
            content.show();
            according.addClass('expanded');
        },
        collapse: function (according) {
            //according.find('.dui-according-content').css('height', '');
            according.find('.dui-according-content').hide();
            according.removeClass('expanded');
        }

    };

    var DeamUISwipe = function (swipe, params) {
        var defaults = {
            transition: 500,
            speed: 3000,
            gap: 0,
        ...
        swipe = $(swipe);
        self.params = $.extend({}, defaults, params || {});
        self.wrapper = swipe.find('.dui-swipe-wrapper');
        self.items = self.wrapper.find('.dui-swipe-item');
        self.page = swipe.find('.dui-swipe-page');
        self.bullets = self.page.find('.dui-swipe-bullet');
        self.buttons = swipe.find('.dui-swipe-button');
            ...
            if (self.page.length > 0) {
                if (self.bullets.length <= 0) {
                    ...
                    for (var i = 0; i < self.items.length; i++) {
                        bulletsHTML += '<div class="dui-swipe-bullet"></div>';
                    }
    ...
    $.fn.swipe = function (params) {
        ...
        return this.each(function () {
            ...
            if (!swipe) {
                params = params || {};
                swipe = new DeamUISwipe(this, params);
    ...
    var DeamUILazyLoad = function (container, params) {
        var defaults = {
            offset: 0,
            delay: 250,
        ...
        self.render = function (ratio) {
            ...
            $.each(self.images, function (i) {
                ...
                if (inview) {
                    ...
                    $this.closest('.dui-page').css('height', $this.closest('.dui-page').height()+'px');
    ...
    $.fn.lazyload = function (params) {
        ...
        return this.each(function () {
            if (!this)
                return;
            var lazyload = new DeamUILazyLoad(this, params);
            if (typeof params === typeof "a") {
    ...
    var DeamUIStars = function (element, params) {
        ...
        var defaults = {
            ...
            labelClass: ['dui-label-default', 'dui-label-primary', 'dui-label-success', 'dui-label-warning', 'dui-label-danger'],
        };
        var self = this;
        self.params = $.extend(defaults, params || {});
        ...
        if (self.params.value > 0) {
            ...
            self.label[0].className = "dui-label " + self.params.labelClass[self.params.value - 1];
        }
        self.star_container.html(starsHTML);
        self.stars = self.star_container.find('i');
        self.label = self.container.find('.dui-label');
        self.stars.click(function (i) {
            ...
            if (self.label.length > 0) {
                ...
                self.label[0].className = "dui-label " + self.params.labelClass[index];
            }
        ...
        self.clear.click(function () {
            ...
            if (self.label.length > 0) {
                ...
                self.label[0].className = "dui-label ";
            }
    ...
    $.fn.stars = function (params) {
        ...
        return this.each(function () {
            ...
            if (!stars) {
                params = params || {};
                stars = new DeamUIStars(this, params);
    ...
    var DeamUIPullToRefresh = function (container, params) {
        var self = this;
        self.container = $(container);
    ...
    $.fn.pullToRefresh = function (params) {
        ...
        return this.each(function () {
            ...
            if (!pulltorefresh) {
                params = params || {};
                pulltorefresh = new DeamUIPullToRefresh(this, params);
    ...
    var DeamUIPullToLoading = function (container, params) {
        var self = this;
        self.container = $(container);
    ...
    $.fn.pullToLoading = function (params) {
        ...
        return this.each(function () {
            ...
            if (!pulltoloading) {
                params = params || {};
                pulltoloading = new DeamUIPullToLoading(this, params);
    ...
    var DeamUIInfinite = function (container, params) {
        var defaults = {
            distance: 40
        };
        var self = this;
    ...
    $.fn.infinite = function (params) {
        ...
        return this.each(function () {
            ...
            if (infinite === undefined) {
                infinite = new DeamUIInfinite(this, params || {});
    ...
    var DeamUITimer = function (container, params) {
        var defaults = {
            startLabel: "距离开始",
            endLabel: "距离结束",
            endText: "活动已结束",
    ...
    $.fn.timer = function (params) {
        ...
        return this.each(function () {
            ...
            if (!timer) {
                timer = new DeamUITimer(this, params || {});
    ...
    var Util = {
        ...
        supportStorage: function () {
            var mod = 'DeamUI.router.storage.ability';
            try {
    ...
    var routerConfig = {
        sectionGroupClass: 'dui-page-group',
        // 表示是当前 page 的 class
        curPageClass: 'dui-page-current',
        // 用来辅助切换时表示 page 是 visible 的,
        // 之所以不用 curPageClass，是因为 page-current 已被赋予了「当前 page」这一含义而不仅仅是 display: block
        // 并且，别的地方已经使用了，所以不方便做变更，故新增一个
        visiblePageClass: 'dui-page-visible',
    ...
    var Router = function () {
        this.sessionNames = {
            currentState: 'DeamUI.router.currentState',
            maxStateId: 'DeamUI.router.maxStateId'
        };
    ...
    Router.prototype._animateElement = function ($from, $to, direction) {
        ...
        var animPageClasses = [
            'dui-page-from-center-to-left',
            'dui-page-from-center-to-right',
            'dui-page-from-right-to-center',
            'dui-page-from-left-to-center'].join(' ');

        var classForFrom, classForTo;
        switch (direction) {
            case DIRECTION.rightToLeft:
                classForFrom = 'dui-page-from-center-to-left';
                classForTo = 'dui-page-from-right-to-center';
                break;
            case DIRECTION.leftToRight:
                classForFrom = 'dui-page-from-center-to-right';
                classForTo = 'dui-page-from-left-to-center';
                break;
            default:
                classForFrom = 'dui-page-from-center-to-left';
                classForTo = 'dui-page-from-right-to-center';
...
//                }
    ...
    function customClickFilter($link) {
        var customRouterFilter = DeamUI.defaults.routerFilter;
    ...
    $(function () {
        ...
        if (!$pages.length) {
            var warnMsg = 'Disable Router  Because Of no .dui-page Elements';
...
/*
        ...
        if (userAgent.indexOf('CK 2.0') <= -1){
            ...
            DeamUI.router = $.router = new Router();
            $(document).on('click', 'a', function (e) {
                var $target = $(e.currentTarget);
                ...
                if ($target.hasClass('back')) {
                    ...
                    DeamUI.router.load(url, ignoreCache);
                }
            });
    ...
    var getPage = function () {
        var $page = $(".dui-page-current");

        if (!$page[0]) {
            $page = $(".dui-page").addClass('dui-page-current');
    ...
    //初始化页面中的JS组件
    DeamUI.initPage = function (page) {
        var $page = getPage();
        if (!$page[0])
            $page = $(document.body);
        var $content = $page.hasClass('dui-content') ?
            $page :
            $page.find('.dui-content');

        //according
        DeamUI.according.init();

        //swipe
        $('.dui-swipe').swipe();

        //搜索
        DeamUI.searchbar.init();

        //懒加载
        
        
        
        $('.dui-content').lazyload();
    ...
    if (DeamUI.defaults.routerShowLoading) {
    ...
    $(document).on($.touchEvents.move, ".dui-header", function (e) {
        e.preventDefault();
    });
    ...
    //     module.exports = DeamUI;

    // } else {
    //     window.DeamUI = DeamUI;
    // }
})();