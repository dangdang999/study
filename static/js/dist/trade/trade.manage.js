define([], function(){
	...
	trade.init = function(uniacid) {
		$('.modal-trade-credit1, .modal-trade-credit2, .modal-trade-consume, .modal-trade-card, .modal-trade-cardsn, .modal-trade-cardconsume').on('click', function(){
			...
			if(type == 'consume') {
				...
				trade.credit(type, uid, title, uniacid);
			} else if (type == 'card') {
				trade.card(uid);
			} else if (type == 'cardsn') {
				trade.card_edit(uid);
	...
	trade.credit = function(type, uid, title,uniacid) {
				...
				'submit': function() {
					...
					$('#form-credit .btn-primary').click(function(){
						...
						if(Validator.isValid()) {
							...
							$.post('./index.php?i='+uniacid+'&r=members&ac=index&op=credit', param, function(data){
								//./index.php?c=mc&a=trade&do=credit
								if(data != 'success') {
									util.message(data, '', 'error');
									return false;
								} else {
